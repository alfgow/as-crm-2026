...
final class ArrendadoresController {
  ...
  public function archivos(Request $req, Response $res, array $params): void {
      ...
      $files = $this->arrendadores->findArchivosByArrendadorId($id);

      $res->json([
          'data' => $files,
          'meta' => ['requestId' => $req->getRequestId(), 'count' => count($files)],
          'errors' => []
      ]);
  }
  ...
  public function addArchivo(Request $req, Response $res, array $params): void {
      ...
      $archivo = $this->arrendadores->addArchivo($id, $tipo, $s3Key);
