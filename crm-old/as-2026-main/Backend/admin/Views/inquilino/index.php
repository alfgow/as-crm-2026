<div class="flex flex-col items-center justify-center mt-16">
    <!-- Título -->
    <h1 class="text-3xl font-bold text-indigo-200 mb-6">Inquilinos</h1>

    <!-- FILTRO / BUSCADOR -->
    <form method="get" action="<?= $baseUrl ?>/inquilino/index" class="flex flex-wrap gap-4 justify-center mb-10">
        <input
            type="text"
            name="q"
    ...
    <div class="w-full max-w-6xl px-4">
        <?php if (!empty($_GET['q'])): ?>
                ...
                <div class="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                    <?php foreach ($inquilinos as $inq): ?>
                        ...
                        if (!empty($profile['nombre'])) {
                            $nombreCompleto = trim($profile['nombre']);
                        } else {
                            $nombreCompleto = trim(
                        ...
                        <div class="bg-white/5 backdrop-blur-md border border-white/20 rounded-2xl p-5 shadow flex flex-col items-center text-center">
                            ...
                            <?php if ($selfieUrl): ?>
                                <img src="<?= htmlspecialchars($selfieUrl) ?>" alt="Foto inquilino"
                            ...
                            <?php else: ?>
                                <div class="w-24 h-24 flex items-center justify-center bg-indigo-600/10 text-indigo-300 text-3xl font-bold rounded-full border-4 border-white/10 ring-2 ring-indigo-500/30 shadow mb-4">
                                    <?= strtoupper(mb_substr($profile['nombre_inquilino'] ?? 'I', 0, 1, 'UTF-8')) ?>
                                </div>
                            <?php endif; ?>

                            <h3 class="text-lg font-bold text-indigo-200 mb-1"><?= htmlspecialchars($nombreCompleto ?: 'Sin nombre') ?></h3>
                            ...
                            <a href="<?= admin_url('/inquilino/' . ($profile['slug'] ?? '')) ?>"
            ...
            <?php else: ?>
                <p class="text-indigo-300 text-center">No se encontraron inquilinos para la búsqueda.</p>
            <?php endif; ?>
        <?php endif; ?>
    </div>
</div>
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        const form = document.querySelector('form[action*="inquilino/index"]');
        if (form) {
            form.addEventListener('submit', () => {
                showLoader("Buscando inquilinos...");
            });
        }
    });
</script>
